...
<?PHP
	...
	if (isset($_POST['deposit'])){
		...
		$sav_receipt = time(); //sanitize($db_link, $_POST['sav_receipt']);
		$sav_payer = sanitize($db_link, $_POST['sav_payer']);
		if($_POST['sav_fixed']){
			$sav_fixed = strtotime(sanitize($db_link, $_POST['sav_fixed']));
		}else {
			$sav_fixed = NULL;
		}
		...
		$sql_insert = "INSERT INTO savings (savtype_id, cust_id, sav_date, sav_amount, sav_receipt, sav_payer, sav_created, sav_slip, user_id) VALUES ('$savtype_id', '$_SESSION[cust_id]', '$sav_date', '$sav_amount', '$sav_receipt', '$sav_payer', '$timestamp', $sav_slip, '$_SESSION[log_id]')";
