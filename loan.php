...
<?PHP
	...
	if (isset($_POST['updatestatus'])){
		...
		$loan_fee_receipt = time();//sanitize($db_link, $_POST['loan_fee_receipt']);
...
<html>
		...
		<script type="text/javascript">
			function firstIssue(form){
				...
				if (status == 2 && issued == 0) {
					...
					loan_fee_receipt = time();//prompt('Please enter Receipt No. for Loan Fee:')
	...
	<body>
		...
		<div class="content_left">
			...
			<form name="loaninfo" method="post" onSubmit="return firstIssue(this)">
				<table id="tb_fields">
					<colgroup>
						<col width="15%">
						<col width="35%">
						...
						<td>
							<input type="text" id="datepicker" name="loan_dateout"
								...
								if($result_loan['loan_issued'] == 1) {
									...
									echo 'value="'.date("d.m.Y", $timestamp).'"';
								}
								?>
		...
		<div class="content_right">
			...
			<form name="loan_repay" action="loan.php" method="post" onSubmit="return validate(this)">
				<table id="tb_fields" style="width:75%">
					...
					<tr>
						<td>Receipt No:</td>
						<td><input type="number" min="1" name="loan_repay_receipt" class="defaultnumber" value="'.time().'" placeholder="for Loan Repayment" /></td>
						<td></td>
				...
				if ($result_loan['loanstatus_id'] == 2 && $loan_default != 0)
					echo '<table id="tb_fields" style="width:75%; background:#a7dbd8">
									...
									<tr>
										<td>Receipt No:</td>
										<td>
											<input type="number" name="fine_receipt" class="defaultnumber" value="'.time().'" placeholder="for Default Fine" />
										</td>
										<td></td>
										<td>
